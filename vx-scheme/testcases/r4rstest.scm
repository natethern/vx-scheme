...
;; We now use this test with our compiler, which elects to open-code
;; +; consequently, this test wouldn't pass, since you can't really 
;; give + a new definition that will stick.  I don't see in the spec
;; where this behavior is required so I've commented out this test
;; for now.
;(define old-+ +)
;(begin (begin (begin)
...
(let ((n (string->number "281474976710655325431")))
  (if (and n (exact? n))
      (test-bignum)))
(newline)
(display "To fully test continuations do:")
(newline)
(display "(test-cont)")
(newline)
(test-sc4)
